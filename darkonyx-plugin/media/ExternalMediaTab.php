 ...
 */
function media_darkonyx_url_insert_form($errors) {
  global $redir_tab, $type;

  $redir_tab = 'darkonyx_external';
  media_upload_header();
  ...
  if (isset($_POST["insertonlybutton"])) {
    ...
    if ($youtube_api) {
      update_post_meta($id, DARKONYX_KEY . "poster_url", $youtube_api["thumbnail_url"]);
      update_post_meta($id, DARKONYX_KEY . "creator", $youtube_api["author"]);
    } else if (strstr($url, "rtmp://")) {
      update_post_meta($id, DARKONYX_KEY . "streamer", str_replace(basename($url), "", $url));
      update_post_meta($id, DARKONYX_KEY . "file", basename($url));
      update_post_meta($id, DARKONYX_KEY . "rtmp", true);
    }
    update_post_meta($id, DARKONYX_KEY . "external", true);
      ...
      <table class="describe">
        <tbody>
          <tr>
            ...
            <td class="field"><input id="insertonly[href]" name="insertonly[href]" value="" type="text" aria-required="true" class="input"></td>
			<span class="description">YouTube and Dailymotions links are also supported!</span>
		  </tr>
          <tr>
            <td></td>
            <td>
